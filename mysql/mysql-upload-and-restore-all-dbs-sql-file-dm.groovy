 ...
 *
 * IMPORTANT: this module relies on docker-machine, installed in the current jenkins server and of course the dbHost to be
 ...
 * @param machineName   docker-machine name where the db server host from file will be restored
 * @param contNameMysql docker mysql container name where the db server host from file will be restored
 */

def extMain(mysqlUser,mysqlRootPass,machineName,contNameMysql) {
        ...
        if (userInput_upload_sql_file == 'Yes') {
            ...
            stage('run SQL file on MYSQL DB container') {
                sh "#!/bin/bash \n" +
                        ...
                        "exec -i ${contNameMysql} /usr/bin/mysql -u ${mysqlUser} --password=${mysqlRootPass}"
            }
        }
    } catch (e) {
